...
@hydra.main(
    version_base=None,
    config_path=str(DEFAULT_CONFIG.parent.relative_to(DIR)),
...
def cli(cfg):
    ...
    if not module:
        raise SyntaxError(
            f"task not recognized. Choices are {', '.join(task_module_map.keys())}"
        )
    ...
    if not func:
        raise SyntaxError(
            f"mode not recognized. Choices are {', '.join(mode_func_map.keys())}"
        )
