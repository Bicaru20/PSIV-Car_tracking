...
class YOLODataset(BaseDataset):
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        try:
            ...
            LOGGER.warning(
                f"{self.prefix}WARNING ⚠️ Cache directory {path.parent} is not writeable: {e}"
            )  # not writeable
        return x
    ...
    def get_labels(self):
        ...
        labels = cache["labels"]
        nl = len(
            np.concatenate([label["cls"] for label in labels], 0)
        )  # number of labels
        assert (
            nl > 0
        ), f"{self.prefix}All labels empty in {cache_path}, can not start training. {HELP_URL}"
        return labels
    ...
    def build_transforms(self, hyp=None):
        if self.augment:
            ...
            transforms = (
                mosaic_transforms(self, self.imgsz, hyp)
                if mosaic
                else affine_transforms(self.imgsz, hyp)
            )
        else:
            transforms = Compose(
