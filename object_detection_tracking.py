...
# Set KMP_DUPLICATE_LIB_OK to True to avoid the following error:
...
# video_cap = cv2.VideoCapture("videos/short.mp4")
video_cap = cv2.VideoCapture("videos/short.mp4")
...
while True:
    ...
    n_frames -= 1
    print(f"Remaining frames: {n_frames}")
    ...
    for track in tracker.tracks:
        ...
        if (
            center_y > start_line_A[1]
            and start_line_A[0] < center_x < end_line_A[0]
            and last_point_y < start_line_A[1]
        ):
            counter_A += 1
            points[track_id].clear()
        elif (
            center_y < start_line_B[1]
            and start_line_B[0] < center_x < end_line_B[0]
            and last_point_y > start_line_A[1]
        ):
            counter_B += 1
            points[track_id].clear()
        # elif (
        #     center_y > start_line_C[1]
        #     and start_line_C[0] < center_x < end_line_C[0]
        #     and last_point_y < start_line_A[1]
        # ):
        #     counter_C += 1
        #     points[track_id].clear()
    ...
    cv2.putText(
        frame, "A", (10, 483), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2
    )
    cv2.putText(
        frame, "B", (530, 483), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2
    )
    # cv2.putText(
    #     frame, "C", (910, 483), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2
    # )
    cv2.putText(
        frame,
        f"{counter_A}",
