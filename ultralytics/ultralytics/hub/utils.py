...
def check_dataset_disk_space(
    ...
    LOGGER.info(
        f"{PREFIX}{data:.3f} GB dataset, {free:.1f}/{total:.1f} GB free disk space"
    )
    if data * sf < free:
...
def sync_analytics(cfg, all_keys=False, enabled=False):
    ...
    if SETTINGS["sync"] and RANK in {-1, 0} and enabled:
        ...
        if not all_keys:
            cfg = {
                k: v for k, v in cfg.items() if v != DEFAULT_CONFIG_DICT.get(k, None)
            }  # retain non-default values
