...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        self.file = file
        self.output_shape = (
            tuple(y.shape) if isinstance(y, torch.Tensor) else (x.shape for x in y)
        )
        self.metadata = {
        ...
        if any((saved_model, pb, tflite, edgetpu, tfjs)):  # TensorFlow formats
            raise NotImplementedError(
                "YOLOv8 TensorFlow export support is still under development. "
                "Please consider contributing to the effort if you have TF expertise. Thank you!"
            )
            assert not isinstance(
                model, ClassificationModel
            ), "ClassificationModel TF exports not yet supported."
            nms = False
            ...
            if tflite or edgetpu:
                ...
                self._add_tflite_metadata(
                    f[8] or f[7], num_outputs=len(s_model.outputs)
                )
            if tfjs:
    ...
    def _export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        ct_model = ct.convert(
            ...
            inputs=[
                ct.ImageType(
                    "image", shape=self.im.shape, scale=1 / 255, bias=[0, 0, 0]
                )
            ],
        )
        bits, mode = (
    ...
    def _export_engine(self, workspace=4, verbose=False, prefix=colorstr("TensorRT:")):
        ...
        ), "export running on CPU but must be on GPU, i.e. `device==0`"
        try:
    ...
    def _export_saved_model(
        ...
        try:
            ...
            import tensorflow as tf  # noqa
        check_requirements(
            ("onnx", "onnx2tf", "sng4onnx", "onnxsim", "onnx_graphsurgeon"),
    ...
    def _export_tflite(
        ...
        if int8:
            ...
            dataset = LoadImages(
                check_dataset(check_yaml(data))["train"], imgsz=imgsz, auto=False
            )
