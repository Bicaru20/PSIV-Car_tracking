...
def yaml_load(file="data.yaml", append_filename=False):
    ...
    with open(file, errors="ignore") as f:
        ...
        return (
            {**yaml.safe_load(f), "yaml_file": str(file)}
            if append_filename
...
def get_settings(file=USER_CONFIG_DIR / "settings.yaml"):
    ...
    defaults = {
        ...
        "uuid": uuid.getnode(),
    }  # device UUID to align analytics
    ...
    with torch_distributed_zero_first(RANK):
        ...
        correct = settings.keys() == defaults.keys() and all(
            type(a) == type(b) for a, b in zip(settings.values(), defaults.values())
        )
        if not correct:
            LOGGER.warning(
                "WARNING ⚠️ Different global settings detected, resetting to defaults. "
